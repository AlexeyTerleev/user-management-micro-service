...
async def me_patch(
    ...
    try:
        ...
        if new_values.username is not None and await users_service.get_user_by_username(new_values.username) is not None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"User with username [{new_values.username}] is already exist",
            )
        if new_values.phone_number is not None and await users_service.get_user_by_phone_number(new_values.phone_number) is not None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"User with phone number [{new_values.phone_number}] is already exist",
            )
        if new_values.email is not None and await users_service.get_user_by_email(new_values.email) is not None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        if new_values.group_name is not None and new_values.group_name != user.group.name:
        ...
        if update_dict.get("password", None) is not None:
            update_dict["hashed_password"] = get_hashed_password(update_dict.pop("password"))
        ...
        updated_user = await users_service.update_user(user.id, UserUpgradeSchema(**update_dict))
        return UserOutSchema(group=GroupOutSchema(**group.dict()), **updated_user.dict())
